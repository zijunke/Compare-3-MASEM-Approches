...
L.values = matrix(rep(0.6,10),10,1,byrow = F)
L.lbound = matrix(c(0,rep(-1,9)),10,1,byrow = F)
L.ubound = matrix(rep(1,10),10,1,byrow = F)
L.labels = matrix(paste('L',1:10,sep=''),10,1,byrow = F)
L.free = L.values!=0
...
# Get results
#----------------------------------------------------------
del.id = which(coefs.fits.multivariate.FIMASEM[,23]>1)
