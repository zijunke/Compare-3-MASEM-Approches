...
matrices.mult <- rCorPop(rho.mult,sigma.mult,
	corr=T,k=reps+500,nonPD.pop="nearPD")
matrices.mult <- matrices.mult[which(sapply(matrices.mult,
	is.positive.definite))][1:reps]

# Step 2: Fit the studied SEM model to each of the simulated correlation vectors
# CFA formulation
# Factor loading matrix
L.values = matrix(c(rep(0.6,15),rep(0,10),rep(0.6,5)),10,3,byrow = F)
L.lbound = matrix(c(0,rep(-1,10),0,rep(-1,3),rep(0,11),rep(-1,4)),10,3,byrow = F)
L.ubound = matrix(c(rep(1,15),rep(0,10),rep(1,5)),10,3,byrow = F)
L.labels = matrix(c(paste('L',1:15,sep=''),rep(NA,10),paste('L',16:20,sep='')),10,3,byrow = F)
L.free = L.values!=0
...
ecov <- mxAlgebra(L%*%t(L) + U , name="expCov")
...
coefs.fits.multivariate.FIMASEM <- as.data.frame(t(sapply(1:reps,function(i) {
	openmxmodel <- mxModel("temp",mxData(matrices.mult[[i]],type="cov",numObs = NpS),
		L,U,ecov, expectation,funML=mxFitFunctionML());
  ...
  oC <- matrices.mult[[i]]
  output <- c(coefs,mySRMR(oC,mC),modelsummary$CFI,openmxfit$output$status[[1]]);
...
# Get results
#----------------------------------------------------------
del.id = which(coefs.fits.multivariate.FIMASEM[,33]>1)
